<template>
  <div>
    ...
    <div v-if="loading">
      <slot name="skeleton">
        <div class="flex flex-col gap-4 animate-fade-in-up">
          <!-- Skeleton filter bar -->
          <div class="bg-white rounded-2xl shadow-panel ring-1 ring-grey-200/70 px-5 py-3">
          ...
          <!-- Skeleton KPI strip -->
          <div class="flex gap-3">
            <div v-for="i in 5" :key="i" class="flex-1 bg-white rounded-2xl shadow-panel ring-1 ring-grey-200/70 px-4 py-3">
          ...
          <div class="flex gap-4">
            <div class="w-5/12 min-w-0 shrink-0 bg-white rounded-2xl shadow-panel ring-1 ring-grey-200/70 p-4">
              <div class="skeleton-shimmer h-[360px] rounded"></div>
            </div>
            <div class="flex-1 bg-white rounded-2xl shadow-panel ring-1 ring-grey-200/70 p-4">
    ...
    <!-- Error banner -->
    <div v-if="error" class="mb-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 flex items-center justify-between animate-fade-in-down">
