...
| **Weight-normalized (⚖)** | A PI compared per kg rather than per pack: competitor price scaled to our pack weight, `PI = BF price ÷ (comp price × BF grams ÷ comp grams)`. Applies only to **Fruits & Vegetables** (our main category), only by **mass** (both sides in g/kg — piece counts are never normalized, because competitors write "1 pcs" for one pack), and only when the two weights differ by more than 10%. Below that the pack sizes count as equal and the PI is left raw. |
| **Weight mismatch** | A Fruits & Vegetables pair whose weight ratio falls outside 1/5–5, or whose piece counts differ by more than 10%: implausible or unverifiable, so either a size or the mapping is wrong. The PI stays raw (never normalized) and the pair is flagged for master-data review. Distinct from Weight-normalized. |
